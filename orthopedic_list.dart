...
import '../BookAppoinment/book_appointment.dart';

class OrthopedicDoctorsDetailsScreen extends StatelessWidget {
  const OrthopedicDoctorsDetailsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Orthopedic Doctors'),
        backgroundColor: AppColors.purpleColor,
      ),
      body: Padding(
        padding: const EdgeInsets.all(8.0),
        child: GetX<OrthopedicDoctorsController>(
          init: OrthopedicDoctorsController(),
          builder: (controller) {
...
class OrthopedicDoctorsController extends GetxController {
  ...
  void fetchDoctors() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance
          .collection('doctors')
          .where('docCategory', isEqualTo: 'Orthopedic')
          .get();
