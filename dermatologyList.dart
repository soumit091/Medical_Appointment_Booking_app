...
class DermatologyDoctorDetailsScreen extends StatelessWidget {
  const DermatologyDoctorDetailsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Dermatology Doctors'),
        backgroundColor: AppColors.purpleColor,
      ),
      body: Padding(
        padding: const EdgeInsets.all(8.0),
        child: GetX<DermatologyDoctorsController>(
          init: DermatologyDoctorsController(),
          builder: (controller) {
...
class DermatologyDoctorsController extends GetxController {
  ...
  void fetchDoctors() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance
          .collection('doctors')
          .where('docCategory', isEqualTo: 'Dermatology')
          .get();
