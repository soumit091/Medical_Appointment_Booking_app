...
class MedicineDoctorsDetailsScreen extends StatelessWidget {
  const MedicineDoctorsDetailsScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Medicine Doctors'),
        backgroundColor: AppColors.purpleColor,
      ),
      body: Padding(
        padding: const EdgeInsets.all(8.0),
        child: GetX<MedicineDoctorsController>(
          init: MedicineDoctorsController(),
          builder: (controller) {
...
class MedicineDoctorsController extends GetxController {
  ...
  void fetchDoctors() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance
          .collection('doctors')
          .where('docCategory', isEqualTo: 'Medicine')
          .get();
